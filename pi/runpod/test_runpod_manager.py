...
class RunpodManagerTests(unittest.TestCase):
    ...
    def set_stock(self):
        ...
        self.manager.config["allowed_data_center_ids"] = ["US-GA-2", "US-CA-2"]
    ...
    def test_no_selected_capacity_reports_blocked_without_create_call(self):
        ...
        self.manager.config["allowed_data_center_ids"] = ["US-CA-2"]
        self.manager._refresh_options = mock.Mock()
    ...
    def test_config_requires_at_least_one_card_and_region(self):
        with self.assertRaisesRegex(ValueError, "at least one GPU"):
            self.manager.update_config({
                "allowed_gpu_ids": [],
                "allowed_data_center_ids": ["US-CA-2"],
            })
        with self.assertRaisesRegex(ValueError, "at least one North American"):
            self.manager.update_config({
